...
void print_network(FILE *file, network *net) {
	...
	print_edges(file, net);
}

void print_edges(FILE *file, network *net) {
	vertex **v = (vertex **) net->vertices;
	int edges_count = 0;
...
int results_writer(network *net, int **edges, double ghost_const,
	...
	*results = fopen(filename, "w");
	...
	if (fprintf(results, RESULTS_FILE_HEADER + "\n") < 0) {
		send_perror("fprintf");
		return 2;
	}
...
int output(network *net, int **edges, double ghost_const,
	...
	if (net->num_of_vertices == 0) {
		...
		if (cluster_sol_writer(net, edges, num_of_clusters) == 2) {
			return 2;
		}

		if (best_solution_writer(net, edges) == 2) {
			return 2;
		}
	}
